...
const v                    = require('../validators/hall.validator');
...
router.post('/',                     requirePermission(PERMISSIONS.HALLS_CREATE),       v.validateCreate, ctrl.create);
router.get('/:id',                   requirePermission(PERMISSIONS.HALLS_READ),         ctrl.getById);
router.patch('/:id',                 requirePermission(PERMISSIONS.HALLS_UPDATE), requireScope(PERMISSIONS.HALLS_UPDATE, resolveHallScope), v.validateUpdate, ctrl.update);
router.put('/:id',                   requirePermission(PERMISSIONS.HALLS_UPDATE), requireScope(PERMISSIONS.HALLS_UPDATE, resolveHallScope), v.validateUpdate, ctrl.update);
...
router.post('/:id/block',            requirePermission(PERMISSIONS.AVAILABILITY_MANAGE), v.validateBlock, ctrl.block);
