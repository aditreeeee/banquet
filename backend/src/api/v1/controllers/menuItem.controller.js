...
const create = async (req, res) => {
    const { categoryId, itemName, description, foodType, unit, basePrice, taxPercent, hsnSacCode, taxType, unitCost } = req.body;
    const item = await menuItemService.create(
        { categoryId, itemName, description, foodType, unit, basePrice, taxPercent, hsnSacCode, taxType, unitCost },
...
const update = async (req, res) => {
    const { itemName, description, basePrice, taxPercent, hsnSacCode, taxType, unitCost, isActive } = req.body;
    const item = await menuItemService.update(
        parseInt(req.params.id, 10),
        { itemName, description, basePrice, taxPercent, hsnSacCode, taxType, unitCost, isActive },
        req.companyId, req.user.user_id
    );
